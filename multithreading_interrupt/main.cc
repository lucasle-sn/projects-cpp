...
void change_status(const char *thread, bool signal) {
    printf("%s, change status: %s -> %s\n", thread, status ? "true" : "false", status ? "false" : "true");
    status = signal;
    printf("Signaling %s\n", signal ? "\"true\"" : "\"false\"");
    pthread_cond_signal(&condition_cond);
}

void set_status(const char *thread) {
    change_status(thread, true);
}

void reset_status(const char *thread) {
    change_status(thread, false);
}
